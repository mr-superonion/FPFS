...
class processBasicDriverConfig(pexConfig.Config):
    ...
    galDir = pexConfig.Field(
        dtype=str, default="small2_psf60", doc="Input galaxy directory"
    )
    noiName = pexConfig.Field(dtype=str, default="var7em3", doc="noise variance name")
    ...
    def setDefaults(self):
        ...
        self.outDir = os.path.join(
            "out%s-%s" % (gnm, self.noiName), "psf%s" % (psfFWHM)
        )
...
class processBasicDriverTask(BatchPoolTask):
    ...
    def process(self, cache, ifield):
        ...
        noiData = pyfits.open(noiFname)[0].data * 10.0
        # same for the noivar model
        powIn = (
            np.load("corPre/noiPows2.npy", allow_pickle=True).item()["%s" % rcut]
            * noiVar
            * 100
        )
        ...
        for ishear in isList:
            ...
            outFname = os.path.join(
                cache.outDir, "fpfs-cut%d-%04d-%s.fits" % (rcut, ifield, ishear)
            )
            if not os.path.exists(outFname) and cache.doFPFS:
                imgList = [
                    galData[
                        i // nn * ngrid + beg : i // nn * ngrid + end,
                        i % nn * ngrid + beg : i % nn * ngrid + end,
                    ]
                    for i in range(ngal)
                ]
